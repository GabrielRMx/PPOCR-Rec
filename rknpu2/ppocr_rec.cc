...
int inference_ppocr_rec_model(rknn_app_context_t* app_ctx, cv::Mat* src_img, ppocr_rec_result* out_result)
{
    int ret;
    rknn_input inputs[1];
    ...
    // Pre Process
    float ratio = src_img->cols / static_cast<float>(src_img->rows);
    int resized_w;
    ...
    cv::Mat resized_img;
    cv::resize(*src_img, resized_img, cv::Size(resized_w, imgH));
    resized_img.convertTo(resized_img, CV_32FC3);
    resized_img = (resized_img - 127.5) / 127.5;
    
    if (resized_w < imgW) {
        cv::copyMakeBorder(resized_img, resized_img, 0, 0, 0, imgW - resized_w, cv::BORDER_CONSTANT, 0);
    }
    ...
    memcpy(inputs[0].buf, resized_img.data, inputs[0].size);
